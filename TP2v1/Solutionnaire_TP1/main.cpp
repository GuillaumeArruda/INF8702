...
#define DEG2RAD(a) (a / 360.0f) * (2.0f*PI);
...
AUX_RGBImageRec *TextureImage;   // les objets-images pour les textures	
...
void dessinerScene()
{
   ...
   // ajuster le fog dynamiquement selon la distance de l'observateur
	...
	glFogf (GL_FOG_START,   rho-(modelSize/2.0f)+2.0);
	glFogf (GL_FOG_END,     rho+(modelSize/2.0f));
...
void appliquerTextures()
{
   ...
   glTexEnvi( GL_TEXTURE_ENV, GL_RGB_SCALE, 1.0);
   ...
   // inscription des variables de textures units dans le nuanceur
...
AUX_RGBImageRec *LoadBMP(char *Filename)
{
   ...
   File=fopen(Filename,"r");
	...
	if (File)
	{
      ...
      MultiByteToWideChar(CP_ACP, 0, Filename, (int)strlen(Filename)+1, Wfilename, sizeof(Wfilename)/sizeof(Wfilename[0]));
...
void init2DTexture(GLint texName, GLint texWidth, GLint texHeight, GLubyte *texPtr)
{
   ...
   glTexImage2D(GL_TEXTURE_2D, 0, 3, texWidth, texHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, texPtr);
}
...
void chargerTextures()
{	
   if (TextureImage=LoadBMP("textures/stonewalldiffuse.bmp")) {
      glGenTextures(1, &texture0);	
      init2DTexture(texture0, TextureImage->sizeX, TextureImage->sizeY, TextureImage->data);
   ...
   if (TextureImage=LoadBMP("textures/rust.bmp")) {
      glGenTextures(1, &texture1);	
      init2DTexture(texture1, TextureImage->sizeX, TextureImage->sizeY, TextureImage->data);
   ...
   if (TextureImage=LoadBMP("textures/3dlabs.bmp")) {
      glGenTextures(1, &texture2);
      init2DTexture(texture2, TextureImage->sizeX, TextureImage->sizeY, TextureImage->data);
