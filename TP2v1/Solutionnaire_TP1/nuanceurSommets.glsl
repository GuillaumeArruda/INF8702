...
void directionalLight( in int i, in vec3 normal, in vec3 eye )
{
   float nDotVP;         // normal . light direction
   float nDotHV;         // normal . light half vector
   float pf;             // power factor
    ...
    // Compute vector from surface to light position
   vec3 VP = normalize( vec3( gl_LightSource[i].position ) );
   nDotVP  = max( 0.0, dot( normal, VP ) );
   
   // Compute the halfvector for local viewer mode
   vec3 halfVector = normalize( VP + eye );
   nDotHV = max(0.0, dot( normal, halfVector ) );
   // otherwise use default light halfvector
...
void flight(in vec3 normal, in vec4 ecPosition, float alphaFade)
{
    ...
    // Pour avoir un modele de lumiere type local viewer
    eye = normalize( -ecPosition3 );
    // sinon la direction de vue est parallele a l'axe z
    ...
    // Calcul de l'illumination "DEUX-FACES"
    Ambient  = vec4 (0.0);
    Diffuse  = vec4 (0.0);
    Specular = vec4 (0.0);
