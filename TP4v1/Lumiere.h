...
public:
   ...
   CLumiere(GLenum lumKD, GLfloat KAr, GLfloat KAg, GLfloat KAb, GLfloat KDr, GLfloat KDg, GLfloat KDb, GLfloat KSr, GLfloat KSg, GLfloat KSb, GLfloat KEr, GLfloat KEg, GLfloat KEb, GLfloat posX, GLfloat posY, GLfloat posZ, GLfloat posW = 1.0, bool estActivee = true, GLfloat spotDirX = 0.0, GLfloat spotDirY = 0.0, GLfloat spotDirZ = -1.0, GLfloat spotExp = 0.0, GLfloat spotCutoff = 180.0);
   /// destructeur par défaut
   ~CLumiere(void);

   /// OPENGL : méthode qui active les paramètres de la lumière si allumée ou la désactive si éteinte.
   void appliquer();  
   ...
   inline void obtenirKE(GLfloat KE[3]) const;
   /// modifie les quatre composantes RGBA de KE
   inline void modifierKE(GLfloat KEr, GLfloat KEg, GLfloat KEb);
...
private:
   /// l'identifiant de la lumière OPENGL associée.
   GLenum lumKD_;
...
///  obtient les quatre composantes RGBA de KE
///
///  @param [in]       KE[3] les trois composantes RGB de KE
///  @author Frédéric Plourde 
///  @date   2007-12-11
///
///////////////////////////////////////////////////////////////////////////////
inline void CLumiere::obtenirKE(GLfloat KE[3]) const
{
   KE[0] = KE_[0];
   KE[1] = KE_[1];
   KE[2] = KE_[2];
...
///  modifie les quatre composantes RGBA de KE
///
///  @param [in]       KEr GLfloat    composante Rouge (peut être NO_OP pour garder ancienne valeur).
///  @param [in]       KEg GLfloat    composante Verte (peut être NO_OP pour garder ancienne valeur).
///  @param [in]       KEb GLfloat    composante Bleue (peut être NO_OP pour garder ancienne valeur).
///
///  @remark les arguments peuvent être mis à NO_OP à l'appel pour empêcher qu'ils soient modifiés.
///  @author Frédéric Plourde 
///  @date   2007-12-11
///
///////////////////////////////////////////////////////////////////////////////
inline void CLumiere::modifierKE(GLfloat KEr, GLfloat KEg, GLfloat KEb)
{
   if (KEr != NO_OP) KE_[0] = KEr;
   if (KEg != NO_OP) KE_[1] = KEg;
   if (KEb != NO_OP) KE_[2] = KEb;
}
...
///  @param [in]       w GLfloat [=1.000000]    la composante en w (pour les lumières directionnelles)
///
///  @return Aucune
///
///  @author Frédéric Plourde 
