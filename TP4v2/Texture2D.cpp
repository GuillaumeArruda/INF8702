...
CTextureAbstraite(GL_TEXTURE_2D, mipmap)
{
   ...
   if (!mipmap_) {
   glTexImage2D(GL_TEXTURE_2D, 0, 3, largeur, hauteur, 0, GL_RGB, GL_UNSIGNED_BYTE, data);
   } else {
      gluBuild2DMipmaps(GL_TEXTURE_2D, 3, largeur, hauteur, GL_RGB, GL_UNSIGNED_BYTE, data);
   }
   ...
   if (nomFichier_ != NULL) {
      if (loadBMP(nomFichier_, &TextureImage)) {
         ...
         if (!mipmap_) {
            ...
            gluBuild2DMipmaps(GL_TEXTURE_2D, 3, TextureImage.tailleX, TextureImage.tailleY, GL_RGB, GL_UNSIGNED_BYTE, TextureImage.data);
         }
      ...
      if (!mipmap_) {
         ...
         gluBuild2DMipmaps(GL_TEXTURE_2D, 3, TextureImage.tailleX, TextureImage.tailleY, GL_RGB, GL_UNSIGNED_BYTE, NULL);
      }
   }
}